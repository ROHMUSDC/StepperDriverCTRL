...
//
//
// Known Bugs:	(NOVEMBER 4th, 2013) Only known bug...Once clock pulses faster 
//				than 390uS, then can only make narrower, not wider
//
// Tasks to Do:	Change Continuous mode to go from CCW-to-0-to-CW
//
// Last Updated: JUNE 16th, 2014 - 18:00
//
//*****************************************************************************

//*****************************************************************************
//TESTED MOTOR CONFIGURATION:
//
// 1.) Portescap 23H118D10B Stepper Motor 
//		Specs: 1A; 1.8 degree per step (200step per revolution) 
//		Wiring: 24V-Grey-Red-Black-Yellow-Ground
// 2.) Vref => ~1.12V
// 3.) MTH  => ~2.48V
//
//*****************************************************************************
...
//	JUMPER SETTINGS
//
// 1.) JUMPER J4 => Jump pin3 to pin4 (Power circuit via USB/LDO: 3.3V)
// 2.) JUMPER J5 => Jump pin3 to pin4 (Power circuit via USB/LDO: 3.3V)
//
//*****************************************************************************

//*****************************************************************************
// Q102 Microcontroller's Hard-wired I/O Pins on the Stepper Motor Driver Board
//
// ================================ ML610Q102 ================================= 
//
...
// Include Path: common;main;irq;timer;clock;tbc;pwm;uart;
	...
	//#include	<setjmp.h>		// Global jump (long jump)
...
static char SINGLE_LED,CONTINUOUS_LED,STEP_MODE_LED,PASS_THRU_RED_LED,PASS_THRU_GREEN_LED,DRIVER_EN_RED_LED;
static char DRIVER_EN_GREEN_LED,FULL_STEP_LED, HALF_STEP_A_LED,HALF_STEP_B_LED,QUARTER_STEP_LED,BUFFER_OE_PIN; 
unsigned int mode = 0;						// 0 to 15...										
unsigned int EncoderPosition = 0;			// 1-4 Quadrature Encoder position
unsigned int previous_encoder_position = 0; 	// 1-4 Quadrature Encoder position
unsigned int button_flag = 0;   		// 1 indicates Button was Pressed...
unsigned int AbsolutePosition = 0; 	// 1-4 Quadrature Encoder position  
unsigned int direction = 0;			// 0=NO CHANGE, 1=CW, 2=CCW
unsigned int previousDirection = 0; 	// 0=NO CHANGE, 1=CW, 2=CCW	 
unsigned int Frequency = 0; 			// For CONTINUOUS MODE

//ADJUSTED THESE TWO BELOW VARIABLE 6/16/2014 to work with PORTESCAP 23H118D 10B Stepper Motor.
static unsigned int CWClkPlseWidth  = 4000;  // <=========CHANGE THIS VALUE TO OPTIMIZE!!! (1000=1.83uS; 4000=7.312ms)
static unsigned int CCWClkPlseWidth = 4000;  // <=========CHANGE THIS VALUE TO OPTIMIZE!!! (1000=1.83uS; 4000=7.312ms)
//===========================================================================
...
//int myLoop,i;
	...
	if (direction != 0)
	{
		...
		case 8:		//SINGLE-STEP MODE...
			//direction =  2; 	// FORCE DIRECTION for TESTING ONLY!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			if (direction == 1){
...
void EncoderDirection(void){
	...
	EncoderPostion();
	//previous_encoder_position = AbsolutePosition;		//ADDED 6/4/2014
...
unsigned int j, ClkHighCounter, ClkLowCounter, HowManyClockCycles;	// 0 to 65,535
...
unsigned int k,WarmUp;				// 0 to 65,535
unsigned int counter, ClockingCount;	// 0 to 65,535
unsigned long LongCounter;			// 0 to 4,294,967,295
	...
	//CONTINUOUS MODE
		...
		//ADJUST CLOCK PULSE WIDTH if CCW...
			...
			else  //For example, not rotation of Encoder...
			{
